...
public:
	...
	// Applies ClearOnEnter, reports FocusOnEnter through |dropFocus|, and fills |command| with
	// OnEnterAction ([$Input] already expanded, empty if there is none). Returns false when Enter
	// is not a commit here, leaving it to be typed as a newline.
	bool HandleEnter(std::wstring& command, bool& dropFocus);
...
private:
	...
	// single-line. Any of the three "on enter" options implies it; Shift+Enter always inserts.
	bool CommitsOnEnter() const
	{
		return !m_OnEnterAction.empty() || m_ClearOnEnter || !m_FocusOnEnter;
	}

	void Clear();

	// Fills |command| with OnEnterAction, [$Input] expanded. False if there is no action.
	bool GetOnEnterCommand(std::wstring& command);
