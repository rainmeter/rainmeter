...
void MeterTextEdit::ReadOptions(ConfigParser::OptionReader& reader)
{
	...
	m_MaxLength = reader.ReadInt<"MaxLength">(0);
	m_Multiline = reader.ReadBool<"Multiline">(false);
	m_ReadOnly = reader.ReadBool<"ReadOnly">(false);

	if (m_TrackInitialText)
	{
		reader.ReadString<"InitialText">(m_Text, L"");
	...
	const bool password = reader.ReadBool<"Password">(false);
	...
	const std::wstring& filterOption = reader.ReadString<"InputFilter">(L"NONE");
	...
	{
		SetInputRegExp(reader.ReadString<"InputRegExp">(L".*"));
	}
	else
	{
	...
	m_InputCase = ReadStringCase(reader, L"InputCase", TEXTCASE_NONE);
	...
	m_SubmitOnEnter = reader.ReadBool<"SubmitOnEnter">(!m_Multiline);
	...
	{
		m_FocusBorderWidth = (FLOAT)reader.ReadFloat<"FocusBorderWidth">(1.0);
	...
	reader.ReadString<"PlaceholderText">(m_PlaceholderText, L"");
	if (!m_PlaceholderText.empty())
	{
		// Only read once there is a placeholder to draw, so a meter without one pays nothing here.
		D2D1_COLOR_F dimmed = m_Color;
		dimmed.a *= 0.4f;
		m_PlaceholderColor = reader.ReadColor<"PlaceholderFontColor">(dimmed);

		m_PlaceholderFontFace = reader.ReadString<"PlaceholderFontFace">(m_FontFace.c_str());
		if (m_PlaceholderFontFace.empty()) m_PlaceholderFontFace = m_FontFace;

		m_PlaceholderFontSize = (FLOAT)reader.ReadFloat<"PlaceholderFontSize">(m_FontSize);
		if (m_PlaceholderFontSize < 0.0f) m_PlaceholderFontSize = m_FontSize;

		// Unset inherits the meter's own style.
		m_PlaceholderStyle = ReadStringStyle(reader, L"PlaceholderStringStyle", m_Style);
	}
	...
	m_CaretColor = reader.ReadColor<"CaretColor">(m_Color);
	...
	m_SelectionColor = reader.ReadColor<"SelectionColor">(D2D1::ColorF(
...
void MeterTextEdit::BindMeasures(ConfigParser::OptionReader& reader)
{
