...
void MeasureSpeedFan::ReadOptions(ConfigParser::OptionReader& reader)
{
	...
	const std::wstring type = reader.ReadString<"SpeedFanType">(L"TEMPERATURE");
	if (_wcsicmp(L"TEMPERATURE", type.c_str()) == 0)
	{
		m_Type = SensorType::Temperature;

		const std::wstring scale = reader.ReadString<"SpeedFanScale">(L"C");
	...
	m_Number = reader.ReadUInt<"SpeedFanNumber">(0);
}
