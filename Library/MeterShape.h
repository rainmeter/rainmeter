...
private:
	...
	bool CreateShape(std::wstring_view definition, ConfigParser& parser, std::wstring_view section, bool& isCombined, size_t keyId);
	bool CreateCombinedShape(ConfigParser& parser, size_t shapeId, std::wstring& options);

	void ParseModifiers(Gfx::Shape& shape, StringParser& modifiers, ConfigParser& parser, std::wstring_view section, bool recursive = false);
	...
	std::wstring ReadShapeOption(ConfigParser& parser, std::wstring_view section, std::wstring key);
