...
void MeterStringEdit::ReadOptions(ConfigParser& parser, const WCHAR* section)
{
	...
	m_FocusOnEnter = parser.ReadBool(section, L"FocusOnEnter", true);
	...
	{
		// The caret is positioned by hit-testing the text layout, which is laid out in the meter's
		// own unrotated space. Undoing the rotation is possible but pointless until there is a real
		// use for editing rotated text.
		LogWarningF(this, L"Meter=StringEdit does not support Angle");
		m_AcceptsInput = false;
	}
...
bool MeterStringEdit::GetOnEnterCommand(std::wstring& command)
{
	if (m_OnEnterAction.empty()) return false;

	command = m_OnEnterAction;
...
bool MeterStringEdit::SelectWordAtCaret()
{
	...
	const UINT32 probe =
		(pos == len || ClassifyChar(m_String[pos]) == CharClass::Space) && pos > 0U ? pos - 1U : pos;
