...
PLUGIN_EXPORT void Initialize (void** data, void* rm)
{
    ...
    *data       = m;
    ...
    // initialize FFT data
    if (
            ((m->m_fftSize=RmReadInt(rm, L"FFTSize", m->m_fftSize)) < 0)
            || (m->m_fftSize & 1)
    )
    {
    ...
    if (
            m->m_fftSize
            && (
                ((m->m_fftOverlap=RmReadInt(rm, L"FFTOverlap", m->m_fftOverlap)) < 0)
    ...
    if ((m->m_nBands=RmReadInt(rm, L"Bands", m->m_nBands)) < 0)
    {
...
PLUGIN_EXPORT void Reload (void* data, void* rm, double* maxValue)
{
    ...
    if (*channel)
    {
        ...
        if (!found)
        {
            ...
            d           += _snwprintf_s(
                                ...
                                L"Invalid Channel '%s', must be an integer between 0 and %d, or one of:",
                                channel,
                                Measure::MAX_CHANNELS-1
                        );
    ...
    if (*type)
    {
        ...
        if (!(iType<Measure::NUM_TYPES))
        {
            ...
            d           += _snwprintf_s(
                                d, (sizeof(msg)+(UINT32)msg-(UINT32)d)/sizeof(WCHAR), _TRUNCATE,
                                L"Invalid Type '%s', must be one of:",
                                type
                        );
    ...
    if (m->m_parent)
    {
        m->m_fftIdx         = min(m->m_parent->m_fftSize/2, m->m_fftIdx);
    }
    else
    {
        m->m_fftIdx         = min(m->m_fftSize/2, m->m_fftIdx);
    }
    // parse band index request
    m->m_bandIdx            = max(0, RmReadInt(rm, L"BandIdx", m->m_bandIdx));
    if (m->m_parent)
    {
        m->m_bandIdx        = min(m->m_parent->m_nBands, m->m_bandIdx);
    }
    else
    {
        m->m_bandIdx        = min(m->m_nBands, m->m_bandIdx);
    }
...
PLUGIN_EXPORT double Update (void* data)
{
    ...
    case Measure::TYPE_DEV_STATUS:
        if (parent->m_dev)
        {
            DWORD state;
            if (
                    (parent->m_dev->GetState(&state) == S_OK)
...
PLUGIN_EXPORT LPCWSTR GetString (void* data)
{
    ...
    case Measure::TYPE_FORMAT:
        if (parent->m_wfx)
        {
            _snwprintf_s(
                    ...
                    _TRUNCATE,
                    L"%dHz %s %dch",
                    parent->m_wfx->nSamplesPerSec,
                    s_fmtName[parent->m_format],
                    parent->m_wfx->nChannels
            );
        }
        break;
    ...
    case Measure::TYPE_DEV_LIST:
        if (parent->m_enum)
        {
            ...
            if (parent->m_enum->EnumAudioEndpoints((parent->m_port==Measure::PORT_OUTPUT) ? eRender : eCapture, DEVICE_STATE_ACTIVE|DEVICE_STATE_UNPLUGGED, &collection) == S_OK)
            {
                WCHAR*              d           = &buffer[0];
                UINT                nDevices;   collection->GetCount(&nDevices);
                for (ULONG iDevice=0; iDevice<nDevices; ++iDevice)
                {
                    IMMDevice*      device      = NULL;
                    IPropertyStore* props       = NULL;
                    if (
                            (collection->Item(iDevice, &device) == S_OK)
                            && (device->OpenPropertyStore(STGM_READ, &props) == S_OK)
                    )
                    {
                        LPWSTR      id          = NULL;
                        PROPVARIANT varName;    PropVariantInit(&varName);
                        if (
                                (device->GetId(&id) == S_OK)
                                && (props->GetValue(PKEY_Device_FriendlyName, &varName) == S_OK)
                        )
                        {
...
HRESULT Measure::DeviceInit ()
{
    ...
    if (*m_reqID)
    {
        ...
        if (hr != S_OK)
        {
            ...
            _snwprintf_s(
                    msg, _TRUNCATE,
                    L"Audio %s device '%s' not found (error 0x%08x).\n",
                    (m_port==PORT_OUTPUT) ? L"output" : L"input",
                    m_reqID,
                    hr
            );
    ...
    {
        hr = m_enum->GetDefaultAudioEndpoint(
                (m_port==PORT_OUTPUT) ? eRender : eCapture,
                eConsole,
                &m_dev
        );
    }
...
void Measure::DeviceRelease ()
{
    ...
    for (int iChan=0; iChan<Measure::MAX_CHANNELS; ++iChan)
    {
        if (m_fftCfg[iChan]) kiss_fftr_free(m_fftCfg[iChan]);    m_fftCfg[iChan] = NULL;
        if (m_fftIn[iChan])  free(m_fftIn[iChan]);               m_fftIn[iChan] = NULL;
        if (m_fftOut[iChan]) free(m_fftOut[iChan]);              m_fftOut[iChan] = NULL;
        if (m_bandOut[iChan]) free(m_bandOut[iChan]);            m_bandOut[iChan] = NULL;
        m_rms[iChan]        = 0.0;
        m_peak[iChan]       = 0.0;
    }
    if (m_bandFreq)
