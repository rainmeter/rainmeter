...
${LangFileString} WRITEERROR              "ポータブル版 Rainmeter をインストールするには、通常ユーザーとしての完全なファイルの読み書き権限を必要とします。$\nRainmeter を「$INSTDIR」へインストールするには、前のメニューへ戻り、「通常インストール」を選択してください。$\n$\nまたは、完全なファイルの読み書き権限のある別の場所を選択してください。"
...
${LangFileString} SETTINGSFILEERROR       "設定ファイル (Rainmeter.ini) の保存先がインストールフォルダーに指定されています。そのままにした場合、制限ユーザーがアプリケーションを実行した際に問題を引き起こす可能性があります。$\n$\nこのファイルをアプリケーションデータフォルダーに移動してもよろしいですか?"
...
${LangFileString} PORTABLEINSTDESC        "この項目を理解している場合のみこちらを選択してください。ショートカットの作成や rmskin ファイルの関連付けは一切行われません。Visual C++ 2010 及び、.NET 2.0 ライブラリを手動でインストールする必要があるかもしれません。"
